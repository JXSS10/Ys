...
LOGGER = logging.getLogger(__name__) # Changed to __name__ for better logging
...
async def get_youtube_info(url: str) -> dict:
    ...
    ydl_opts = {
        ...
        'cookiejar': cookies,  # Use loaded cookies here
        'http_headers': {
    ...
    if ydl_opts['cookiejar'] is None:
        ...
        LOGGER.debug("Cookies are not being used for info fetching as they are not loaded.")
    else:
        LOGGER.debug("Cookies will be used for info fetching.")
...
async def download_progress_hook_async(d: dict, status_message: Message, session_data: dict):
    ...
    if not user_id:
        return  # Should not happen if session is valid
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if is_playlist:
                ...
                for entry in info_dict.get('entries', []):
                    if not entry:
                        continue  # Skip potential None entries on error
                    filepath = None
                    req_dl = entry.get('requested_downloads')
                    if req_dl:
                        filepath = req_dl[-1].get('filepath')  # Most reliable path after processing
    ...
    if media_type == 'video':
        ...
        for h in sorted_heights:
            ...
            if h >= 720 and not tiers['medium']:
                tiers['medium'] = found_formats[h]  # Can overlap with high if only >720 exists
            if h <= 480 and not tiers['low']:
                tiers['low'] = found_formats[h]  # Takes highest available <= 480p
        ...
        audio_formats = sorted(
            ...
            key=lambda x: x.get('abr') if x.get('abr') is not None else 0, reverse=True #Fixed: Handle NoneType for abr
        )
...
async def callback_query_handler(client: Client, callback_query: CallbackQuery):
    ...
    if not session_data or session_data.get('status_message_id') != message.id:
        ...
        try:
            await message.edit_reply_markup(reply_markup=None)  # Try remove buttons
        except:
            pass
        return
